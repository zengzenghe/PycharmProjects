...
def AddWorldCity(input_file):
    ...
    for index, row in df.iterrows():
        ...
        if row['code'] == 0 and row['nationOrcity'] != '中国':
            if row['nationOrcity'] not in place_code.keys() and row['nationOrcity'] not in provinceMap.keys() and row[
                'nationOrcity'] not in standard_place_dic.keys():
        ...
        if row['code'] != 0 and row['nationOrcity'] not in place_code.keys() and row[
            'nationOrcity'] not in provinceMap.keys() and row['nationOrcity'] not in standard_place_dic.keys():
            txt = str(row['nationOrcity']) + '\n'
